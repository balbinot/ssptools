...
class Kroupa():
    ...
    def pintegral(self):
        ...
        for i in range(len(a)):
            ...
            print('a={:.2f}, piece=[{:.2f}, {:.2f}], limits=[{:.2f}, {:.2f}]'.format(a[i], mlim[i], mlim[i+1], mlim[i], mlim[i+1]))
        I = np.array(I)
        I2 = np.array(I2)/I
        return (I, I2)
    ...
    def integral(self, xmin, xmax):
        ...
        if xmin < mlim[0]:
            raise ValueError('xmin is less than the lower domain boundary of {:.2f}'.format(mlim[0]))
        if xmax > mlim[-1]:
            raise ValueError('xmax is larger than the upper domain boundary of {:.2f}'.format(mlim[-1]))
        ...
        if imin==imax:
            ...
            for i in range(imin, imax):
                ...
                if debug==True:
                    print('a={:.2f}, piece=[{:.2f}, {:.2f}], limits=[{:.2f}, {:.2f}]'.format(a[i], mlim[i], mlim[i+1], XMIN, XMAX))
    ...
    def _getmass(self, x, slope, xmin, xmax):
        ...
        if slope==1:
            ...
            A = (1./(1-slope)) * (pow(xmax, 1.-slope) - pow(xmin, 1-slope))
...
if __name__=='__main__':
    ...
    eep, m, logT, logL, g, r, i = np.loadtxt('/home/eb0025/Dropbox/sptools/MIST_iso_5b0533fab00b3.iso.cmd',
    ...
    for mmin, mmax in zip(m[0:-1],m[1:]):
        ...
        print("{:.4f} {:.4f} {:.4f} {:.4f} {:.4f}".format(mmin, mmax, mmax-mmin, I, I2))
    ...
    p.hist(np.log10(mass), bins=bins, normed=True, log=True,
